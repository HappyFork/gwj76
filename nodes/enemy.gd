...
const FREEZE_NOISE = preload("res://assets/Sound/SFX/GWJ 2.1_Freeze.mp3")
...
func _ready():
	...
	wtimer.start()
	# Ensure the main AudioStreamPlayer3D node is found
	assert(sfx != null, "AudioStreamPlayer3D not found")
...
func decide(unfreeze = false):
	...
	if state == States.FROZEN and not unfreeze:
		return
